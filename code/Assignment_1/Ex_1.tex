...
\begin{document}
\section*{Assignment 1 Alvaro Méndez (2185566)}

\section{Section 1: Scottish Lakes}
...
\begin{verbatim}
  The lake with the largest volume is Loch Ness with a volume of 7.45 km^3.
The lake with the smallest volume is Loch Shin with a volume of 0.35 km^3.
The lake with the largest area is Loch Lomond with an area of 71 km^2.
The lake with the smallest area is Loch Katrine with an area of 12.4 km^2.
The two lakes with the largest area are: 
         Name Area_km2
1   Loch Ness       56
2 Loch Lomond       71
The total area of the  13  lakes is 372 km^2).
\end{verbatim}

\section{Section 2: 100 m Athletics Records}
\subsection{Men's 100m}
\begin{lstlisting}[language=R]
library(rvest)
library(tidyverse)
library(ggplot2)
...
ggplot(fastest_per_country, aes(x = Date, y = order(Best_Time), color = Country)) +
  geom_line(size = 1) +
  geom_point() +
  scale_y_reverse() +
  theme_bw() +
  labs(title = "Fastest 100m Runner per Country Over Time",
...
ggplot(men100m_tbl, aes(x = fct_infreq(Country), fill = as.factor(Decade_Label))) +
  ...
  coord_flip() +
  theme_minimal() +
  labs(title = "Top 100m Times by Country and Decade",
       x = "Country",
       y = "Number of Records",
       fill = "Decade")
\end{lstlisting}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.8\textwidth]{images/plot-64.png}
%\caption{plot-64}
\end{figure}
\begin{figure}[ht]
\centering
\includegraphics[width=0.8\textwidth]{images/plot-65.png}
%\caption{plot-65}
\end{figure}

\subsection{Women's 100m}
\begin{lstlisting}[language=R]
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.8\textwidth]{images/plot-66.png}
%\caption{plot-66}
\end{figure}
\begin{figure}[ht]
\centering
\includegraphics[width=0.8\textwidth]{images/plot-67.png}
%\caption{plot-67}
\end{figure}
